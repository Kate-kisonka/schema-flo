...
export default function Breathing478() {
  ...
  const totalRounds = 4;
  const steps = [
    { label: "Готова?", duration: 0, color: T.muted, hint: "" },
    { label: "Вдох", duration: 4, color: T.green, hint: "через нос, медленно" },
    { label: "Задержка", duration: 7, color: T.yellow, hint: "не дышать" },
    { label: "Выдох", duration: 8, color: T.blue, hint: "через рот, со звуком" },
  ];
  ...
  return (
    <div style={{ textAlign: "center", marginTop: 18 }}>
      {step > 0 && <div style={{ fontSize: 10, color: T.muted, marginBottom: 7 }}>Цикл {round+1} из {totalRounds}</div>}
      <div style={{ position: "relative", width: 110, height: 110, margin: "0 auto 14px" }}>
        ...
        <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", transform: `scale(${scale})`, transition: "transform 1s ease" }}>
          ...
          <div style={{ fontSize: step>0?10:11, color: T.sub, marginTop: step>0?2:0 }}>{cur.label}</div>
        </div>
      </div>
      {step > 0 && <div style={{ fontSize: 11, color: T.muted, marginBottom: 11, fontStyle: "italic" }}>{cur.hint}</div>}
      <button onClick={step===0?start:stop} style={{ padding: "8px 22px", borderRadius: 16, border: `1px solid ${T.border}`, background: step>0?T.orange:T.text, color: "#fff", cursor: "pointer", fontFamily: T.font, fontSize: 13 }}>
        {step > 0 ? "Стоп" : "Начать"}
      </button>
      {step===0&&round===0&&<div style={{fontSize:10,color:T.muted,marginTop:7}}>4 цикла · ~1.5 минуты</div>}
      {step===0&&round>0&&<div style={{fontSize:13,color:T.green,marginTop:8}}>✓ Готово! Как ты?</div>}
    </div>
  );
}
